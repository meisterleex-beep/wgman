...
def dashboard(handler):
    ...
    body = ("<h1>wgman</h1><div class='sub'>WireGuard manager for OpenWrt routers</div>"
            ...
            "<div class='kv'><span class='k'>API token</span><span class='v'>" + html.escape(read_file(TOKEN_FILE)) + "</span></div>"
            "</div></div>"
            "<div class='actions'>"
...
"async function rotateToken(){if(!confirm('Rotate API token? Old token will stop working.'))return;"
"const j=await post('/api/token',{action:'new'});"
"if(j.ok){alert('New API token: '+j.token);location.reload();}else alert(j.message);}"
